...
#	  
#   Revisions: 
#   19JAN2017 -- Update Assignment of ART trajectory (Giganti)
#
#   
#
#############################################################
##The code below was modified by Giganti to improve the ART simulation code
##The substantial recommended changes to datasim.R are located at the end of the file and should be clearly noted
##Could not locate the original CCASAnet dataset used to generate simulated data in previous iterations
##Giganti used CCASAnet data from 18SEP2014


#Here are a few minor issues that could be improved in data simulation with additional resources:
#Simulated death rate lower than observed
#Simulated virologic failure rate higher than observed
#Simulated regimen change rate slightly higher than observed
#Losing approximately additional 3% of cohort due to exclusion criteria (Earlier ART start dates than expected (pre 1996) + patients start younger than age 18 )
#ART regimen trajectories are being shortened (if greater than censor date), but never lengthed.  This results in fewer number of total switches.
#This code assigns a baseline regimen to every patient.  The original data had some patients who never received ART.
...
setwd("~/Documents/CCASAnet/Data/datasim")
...
## READ IN DATA FILES
readtables <- c("basic","art","follow","lab_cd4","lab_rna","visit")

## data is on encrypted hard-drive (not dropbox)
datalocation <- "~/Documents/CCASAnet/Data/Archived/18SEP2014"
...
d2<-merge(d,lab_cd4,by=c("site","patient"),all.y=TRUE)
...
#19JAN2017
#Giganti modification starts here
#####################################################
#Here is what I believed was intended for the simulated data:
#####################################################
##Randomly assign the first regimen by permuting among all actual first dispensation (this is done earlier)
##Randomly assign the rest of the drugs dispensed as 'chunks'; that is, select an entire regimen history among all regimen histories that start with the initial dispensation
##Use the same lengths of time on each regimen (plus a jitter), but UPDATE them to correspond to the treatment start date for that patient

#####################################################
#Here is what I believed was previously incorporated for the simulated data:
#####################################################
# -- Patients were being assigned the wrong regimen history.  Rather than being assigned the regimen history with the same initial regimen, many patients were assigned regimen history of a patient that initiated a different regimen. 
# -- This regimen history was added to the first dispensation that had previously been assigned, rather then being merged with it.  This led to many patients having 2 different regimens at the same start date.
# -- Due to jittering of the start dates for regimens, many patients were showing almost immediate changes between the original first dispensation and this *additional* first dispensation. This led to very high treatment modification rates.
# -- Different jitters for art_ed of one regimen and art_sd of the next.  This led to some future start dates being before the end of the previous regimen

#####################################################
#Here is a summary of my edits below for the simulated data:
#####################################################
#Re-order art_sim variable by art_id rather than patient_id. This should allow the assignment of 'actual_patient' to correspond to the correct regimen
#Merge in entire regimen histories and eliminate confusing code that breaks up art_change into time 0 and non-time 0 datasets
#Add jitter after merging, and sync up jitter amount between art_ed and subsequent art_sd
#Update art_sd and art_ed variables for each regimen to correspond to overall first haart_d
#For confidentiality reasons, replace all starting regimens (and subsequent trajectories) that occured in less than 10 patients

#List of startregimens occuring less than 10 times
starting.reg.LT10<-names(table(art_sim$art_id)[table(art_sim$art_id)<11])
starting.reg.GT10<-names(table(art_sim$art_id)[table(art_sim$art_id)>10])

#Randomly sample new regimens from the list of regimens that occured more than 10 times
art_sim$art_id<-ifelse(art_sim$art_id%in%starting.reg.LT10,sample(starting.reg.GT10,nrow(art_sim),replace=TRUE),as.character(art_sim$art_id))


art_sim_temp<-art_sim[order(art_sim$art_id),]

set.seed(2016)

## assign random patient trajectory based on same baseline regimen
x <- with(art_sim,split(patient,art_id))
x1 <- lapply(seq_along(x),function(i) sample(art_change0$actual_patient[art_change0$art_id %in% names(x)[[i]]],size=length(x[[i]]),replace=TRUE))

art_sim_temp$actual_patient <- with(art_sim_temp,unlist(x1,art_id))
art_sim<-art_sim_temp[order(art_sim_temp$patient),]
colnames(art_sim)[colnames(art_sim)=="art_sd"]<-"haart_d"

art_change2<-art_change
names(art_change2)[names(art_change2)=="patient"] <- "actual_patient"
art_change2$art_sd<-art_change2$art_ed<-NA
art_change2$haart_d<-art_change2$site<-NULL
art_sim1<- merge(art_sim[c("patient","actual_patient","haart_d","site")],art_change2,by="actual_patient")


art_sim1<-art_sim1[order(art_sim1$patient,art_sim1$art_start2sd),]




## jitter time between regimens
## Need to preserve ordering of regimens
##Key changes include negative jittering and different jitters for sd/ed
####Use jitter time of ed for next sd
####Make sure that new ed will be greater than sd
## For loop not ideal, but quick
## Assigning same jitter to ensure greater ed is not perfect, but only small number of instances ~100
myjitter.1<-rep(0,nrow(art_sim1))
myjitter.2<-sample(-5:5,nrow(art_sim1),replace=TRUE)

duplication.indicator<-duplicated(art_sim1$patient)
for (i in 1:nrow(art_sim1)){
	if (duplication.indicator[i]){
		myjitter.1[i]<-myjitter.2[i-1]
	}
	diff.temp<-art_sim1$art_start2sd[i]+myjitter.1[i] - art_sim1$art_start2ed[i]-myjitter.2[i]
		if (diff.temp>=0 & is.na(diff.temp)==F) {
			myjitter.2[i]<-myjitter.1[i]
		}
	}
...
art_sim1$art_sd <- with(art_sim1,haart_d + art_start2sd +myjitter.1)
art_sim1$art_ed <- with(art_sim1,haart_d + art_start2ed + myjitter.2)
...
#################################################
#End Giganti modification to datasim.R code
#################################################

write.csv(basic_sim,"output/basic_sim.csv",na="",row.names=FALSE)
write.csv(art_sim3[c('patient','site','art_id','art_sd','pi',
                     'nnrti1','nnrti2','nnrti','nrti','t20','ccr5','ii1','ii2',
                     'rtv_drug','numdrugs','art_class','art_ed','art_rs')],"output/art_sim.csv",na="",row.names=FALSE)
write.csv(follow_sim,"output/follow_sim.csv",na="",row.names=FALSE)
write.csv(lab_cd4_sim,"output/lab_cd4_sim.csv",na="",row.names=FALSE)
write.csv(lab_rna_sim,"output/lab_rna_sim.csv",na="",row.names=FALSE)
write.csv(visit_sim,"output/visit_sim.csv",na="",row.names=FALSE)
